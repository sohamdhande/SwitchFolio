...
import type { SwitchfolioOptions, SwitchfolioResult, SwitchfolioProject } from "./types"

export function useSwitchfolio(options: SwitchfolioOptions): SwitchfolioResult {
  const { apiKey, username, viewSlug, baseUrl = "https://switchfolio.app" } = options

  const [data, setData] = useState<SwitchfolioProject[]>([])
